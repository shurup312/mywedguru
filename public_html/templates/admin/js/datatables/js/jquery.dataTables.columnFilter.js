...
(function ($) {
    ...
    $.fn.columnFilter = function (options) {
        ...
        function fnCreateInput(oTable, regex, smart, bIsNumber, iFilterLength, iMaxLenght) {
            ...
            var input = $('<input type="text" style="width:100%;margin-left: -1.5px;" class="' + search_init + sCSSClass + '" value="' + inputvalue + '" rel="' + i + '"/>');
        ...
        function fnCreateDateRangeInput(oTable) {
            ...
            //th.html(_fnRangeLabelPart(0));
            var sFromId = oTable.attr("id") + '_range_from_' + i;
            var from = $('<input type="text" readonly="readonly" class="date_range_filter" id="' + sFromId + '" rel="' + i + '"/>');
            from.datepicker({
				dateFormat: 'dd.mm.yy',
				firstDay: '1',
            ...
            //th.append(_fnRangeLabelPart(1));
            var sToId = oTable.attr("id") + '_range_to_' + i;
            var to = $('<input type="text" readonly="readonly" class="date_range_filter" id="' + sToId + '" rel="' + i + '"/>');
            //th.append(to);
            //th.append(_fnRangeLabelPart(2));
        ...
        function fnCreateColumnMultiSelect(oTable, aData, iColumn, nTh, sLabel, bRegex, oSelected) {
            bRegex = true;
            if (aData == null)
                aData = _fnGetColumnValues(oTable.fnSettings(), iColumn, true, false, true);
            var index = iColumn;
            ...
            var r = '<select style="width: 100%;" class="search_init select_multi_filter" multiple rel="' + i + '"><option value="" class="search_init">' + sLabel + '</option>';
            var j = 0;
            var iLen = aData.length;
            for (j = 0; j < iLen; j++) {
                if (typeof (aData[j]) != 'object') {
            ...
            var select = $(r + '</select>');
            nTh.html(select);
            nTh.wrapInner('<span class="filter_column filter_multi_select" />');
            select.change(function () {
                if ($(this).val() != null) {
                    $(this).removeClass("search_init");
                } else {
                ...
                var val_str = unescape($(this).val());
                
                if (val_str.substr(0,1) == ',')
                {
                    val_str = val_str.substr(1,val_str.lenght);
                }
                var val_str_arr = val_str.split(',');
                oTable.fnFilter(val_str_arr.join('|'), iColumn, true); //Issue 25
                fnOnFiltered();
            });
        ...
        function fnCreateMultiSelect(oTable, aData, bRegex, oSelected) {
            var oSettings = oTable.fnSettings();
            if (aData == null && oSettings.sAjaxSource != "" && !oSettings.oFeatures.bServerSide) {
                // Add a function to the draw callback, which will check for the Ajax data having 
                // been loaded. Use a closure for the individual column elements that are used to 
                ...
                oSettings.aoDrawCallback.push({
                    "fn": (function (iColumn, nTh, sLabel) {
                        return function () {
                            ...
                            // data has been loaded.
                            if (oSettings.iDraw == 2 && oSettings.sAjaxSource != null && oSettings.sAjaxSource != "" && !oSettings.oFeatures.bServerSide) {
                                return fnCreateColumnMultiSelect(oTable, null, _fnColumnIndex(iColumn), nTh, sLabel, bRegex, oSelected); //Issue 37
                            }
                        };
                    })(i, th, label),
                    "sName": "column_filter_" + i
            ...
            // Regardless of the Ajax state, build the select on first pass
            fnCreateColumnMultiSelect(oTable, aData, _fnColumnIndex(i), th, label, bRegex, oSelected); //Issue 37
                ...
                if (aoColumn != null) {
                    ...
                    switch (aoColumn.type) {
                        ...
                        case "multi-select":
                            if (aoColumn.bRegex != true)
                                aoColumn.bRegex = false;
                            fnCreateMultiSelect(oTable, aoColumn.values, aoColumn.bRegex, aoColumn.selected);
                            break;
                        case "number-range":
