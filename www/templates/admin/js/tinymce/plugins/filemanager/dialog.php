...
<!-- header div end -->
	...
	<div class="span12">	    
	    <?php
        //die ($current_path.$rfm_subfolder.$subdir);
        if(@opendir($current_path.$rfm_subfolder.$subdir)===FALSE){ ?>
	    <br/>
		...
		foreach ($files as $file_array) {
			...
			$file=$file_array['file'];
				...
				$src = $cur_dir . rawurlencode($file);
				$src=substr($src, strlen($_SERVER['DOCUMENT_ROOT']));
		...
		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<div class="img-precontainer">
				    ...
				    <div class="img-container">
					    <span></span>
  <?php //$src_thumb=substr($src_thumb, strlen($_SERVER['DOCUMENT_ROOT']));?>
  <?php //$mini_src=substr( $mini_src, strlen($_SERVER['DOCUMENT_ROOT']));?>
