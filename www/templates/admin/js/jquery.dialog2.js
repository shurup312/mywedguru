...
(function($) {
    ...
    var Dialog2 = function(element, options) {
        ...
        $(handle).on('click', "a.ajax", function(event){
    ...
    Dialog2.prototype = {
        ...
        __updateMarkup: function() {
            ...
            var actions = $(".form-actions", e).css({ position: "fixed", left: "-9999px", height: "1px" });
            ...
            if (buttons.length) {
                ...
                buttons.each(function() {
                    ...
                    options.buttons[name] = {
                        ...
                        click: function(event) {
                            //if (button.is("a")) { window.location = button[0].href }  //убрал чтобы ссылки в блоке form-actions грузились в этом же модальном окне
        ...
        __remove: function() {
            ...
            this.__handle.removeData("dialog2").parent().parent().parent().remove();
        }, 
        ...
        options: function(options) {
            ...
            if (options.class) {
                parentHtml.parent().addClass(options.class);
            }
            if (buttons) {
            ...
            if (__boolean(options.removeOnClose)) {
                ...
                if (options.removeOnClose) {
                    handle.closest('.modal').bind("dialog2.closed", this.__removeDialog);
                }
            }
	...
	function dialogLoad(url, completeCallback, beforeSendCallback) {
		...
		jQuery.ajax({
			...
			type: type || this.data('options').type,
